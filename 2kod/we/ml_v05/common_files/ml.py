...
class DR_Pcoord:
    ...
    def proc_cpp_data(self, scale=True):
        ...
        self.data = np.atleast_2d(np.loadtxt(self.data))[:,1:]
...
if __name__ == "__main__":
    ...
    with open(os.path.join(pkl_path, 'pca.pkl'), 'rb') as handle:
        ml = pickle.load(handle)
    #plt.scatter(ml.proj[:,0], ml.proj[:,1], c=ml.frames, s=5)
    # load transform new loaded data test
    ml2transform = DR_Pcoord(input_file, lagtime=10, n_components=2, angle_data=True)
    ml2transform.proc_cpp_data()
    new_proj = ml.model.transform(ml2transform.data)
    np.savetxt("PC1.dat", new_proj[:,0])
    np.savetxt("PC2.dat", new_proj[:,1])

    # read binary pkl file
    with open(os.path.join(pkl_path, 'tica.pkl'), 'rb') as handle:
        ml = pickle.load(handle)
    #plt.scatter(ml.proj[:,0], ml.proj[:,1], c=ml.frames, s=5)
    # load transform new loaded data test
    ml2transform = DR_Pcoord(input_file, lagtime=10, n_components=2, angle_data=True)
