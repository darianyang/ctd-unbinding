...
# calc weighted avgs
#python $WEST_SIM_ROOT/common_files/weighted_dmat_pcoord.py m1w184_m2_dmat.dat $WEST_SIM_ROOT/common_files/m1w184_m2_weights.txt w_avgs_m1.txt
#python $WEST_SIM_ROOT/common_files/weighted_dmat_pcoord.py m2w184_m1_dmat.dat $WEST_SIM_ROOT/common_files/m2w184_m1_weights.txt w_avgs_m2.txt
...
paste <(cat tIC1.dat) <(cat tIC2.dat) <(cat pcoord.dat | tail -n +2 | awk '{print $6}') > $WEST_PCOORD_RETURN
...
cat pcoord.dat | tail -n +2 | awk {'print $4'} > $WEST_N_NC_RETURN
cat pcoord.dat | tail -n +2 | awk {'print $5'} > $WEST_N_NNC_RETURN
cat pcoord.dat | tail -n +2 | awk {'print $6'} > $WEST_MIN_DIST_RETURN

# save PCs and tICs
cat PC1.dat > $WEST_PC1_RETURN
cat PC2.dat > $WEST_PC2_RETURN
cat tIC1.dat > $WEST_TIC1_RETURN
cat tIC2.dat > $WEST_TIC2_RETURN
...
rm -f md.in $PRMTOP seg.nfo seg.pdb *.dat* *.txt
