...
def concat_iter(trace, top, out_path=None):
    ...
    if out_path:
        ...
        with mda.Writer(out_path, protein.n_atoms) as W:
            for ts in tqdm(trace_traj.trajectory):
                W.write(protein)
...
trace = trace_walker((400, 1), "west.h5")
concat_iter(trace, f"common_files/2kod_m01_dry.prmtop", out_path="traced_traj/400_1.ncdf")
